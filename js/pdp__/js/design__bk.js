...
mst(document).ready(function ($) {
    ...
    if (($('#edit_id').val() != '')||($('#wid').val() != '')||($('#cid').val() != '')) {
        if ($('.design-color-image li.active').length > 0) {
    ...
    $('.tab_design_image .design').click(function () {
        $('.tab_design_image .active').removeClass("active");
        $(this).addClass("active");
        $('#select_image .content_designs').show();
        $('#select_image .content_upload').hide()
    });
    $('.tab_design_image .upload').click(function () {
        $('.tab_design_image .active').removeClass("active");
        $(this).addClass("active");
        $('#select_image .content_designs').hide();
        $('#select_image .content_upload').show()
    });
    update_inlay_option();
    resetTextForm();
    ...
    $('#icon_list, #lists_img_upload').on('click', 'img', function () {
    ...
    $(".add-to-cart333 .btn-cart").click(function () {
        var f = $('#skin_url').val(),
            g = $('#base_dir').val(),
            ml = $('#media_url').val(),
            k = $('#qty').val(),
    ...
    function changeToEditForm(a) {
        ...
        var b = a.css("text-shadow");
        if (b != 'none') {
            $('.font_shape .h-shadow,.font_shape .v-shadow,.font_shape  .blur,.font_shape  .color').show();
            var c = b.split(" ");
            $('#h-shadow').val(parseInt(c[3]));
            $('#v-shadow').val(parseInt(c[4]));
            $('#t-blur').val(parseInt(c[5]));
            $('#font_outline_color > div').css("backgroundColor", c[0] + c[1] + c[2]);
            if (!$('#use_shadow').is(":checked")) {
                $('#use_shadow').click()
            }
        } else {
