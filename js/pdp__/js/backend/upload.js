...
mst(document).ready(function($) {
	...
	var skinUrl = $('#skin_url').val() + 'adminhtml/default/default/designpro/image/';
			...
			if(this.items.length > 0){
				...
				var uId = "";
	 			for(var i = 0; i<this.items.length; i++){
					...
					var sampleIcon = '<img src="'+ skinUrl +'image.png" />';
					var errorClass = "";
					
					if(typeof this.items[i] != undefined){
                        //console.log(this.items[i]);
						if(self._validate(this.items[i].type) <= 0 && self._fontValidate(this.items[i].name) == false) {
							sampleIcon = '<img src="'+ skinUrl +'unknown.png" />';
							errorClass =" invalid";
						} 
						html += '<div class="dfiles'+errorClass+'" rel="'+uId+'"><h5>'+sampleIcon+this.items[i].name+'</h5><div id="'+uId+'" class="progress" style="display:none;"><img src="' + skinUrl + 'ajax-loader.gif" /></div></div>';
					}
				}
