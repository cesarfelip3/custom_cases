...
mst(document).ready(function($){
    ...
    var cart_i = 0;
    $('.item-options input.image_final').each(function(){
        cart_i++;
        ...
        for(i=7;i<img_first.length;i++){
            ...
            if(label[1]>0){
                if(check1){
                    $('#info_'+id).append('<p>Size selected</p><ul></ul>');
                    check1 = false;
                }
                $('#info_'+id).children('ul').append('<li><label>'+label[0]+'  </label><span>: '+label[1]+' item(s)</span></li>');
            };
        };
        //$(this).html("");
        //$(this).parent().parent().next().next().find('.price').html('$'+img_first[6]);
        //$(this).parent().prev().find('a').attr("href",$('#pdp_uri_key').val()+'?id='+id);
        //$(this).parent().parent().next().find('a').attr("href",$('#pdp_uri_key').val()+'?id='+id);
        $('#info_'+id).append('<div style="display: none;"><div id="img_f_result_'+id+'" class="img_result"></div></div><div style="display: none;"><div id="img_b_result_'+id+'" class="img_result"></div></div>');
        ...
        $(this).parent().parent().prev().find("img").attr("title","Click to view").attr("src",img_first[1]).attr("height",'auto').wrap('<a class="inline group'+cart_i+'" href="#img_f_result_'+id+'"></a>');
